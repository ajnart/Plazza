...
std::vector<std::string> Reception::split(const std::string &s, char block) {
    ...
    while (std::getline(stream, token, block)) tokens.push_back(token);
    return tokens;
}
    ...
    for (int i = 0; i < 3; ++i) {
        if (i == 0) {
            ...
            if (command[i] != "S" && command[i] != "M" && command[i] != "L" &&
                ...
                throw(PlazzaException("Invalid pizza size. Valid size are [S/M/L/XL/XXL]"));
            return true;
        } else {
...
bool Reception::checkLine() {
    ...
    for (auto const &token : Split) {
        ...
        command.erase(
            ...
            command.end());
        if (command.size() != 3) throw(PlazzaException("Invalid size"));
        cmdChecker(command);
        ...
        for (int i = 0; i < 3; ++i) {
            if (i == 0) {
                for (auto const &elem : _Pizzas) {
                    if (elem.first == command[0]) newCommand.type = elem.second;
                }
            } else if (i == 1) {
                for (auto const &elem : pizzaSizes) {
                    if (elem.first == command[1]) newCommand.size = elem.second;
                }
            } else
